...
<p><code>harness</code> is not a special case. Train a word-embedding model on the archive's
...
the ones this book is about.</p>
"""

CH7_DRIFT_B = """
<p>Read the two right-hand columns as a pair and the pattern is not that these words got more
advanced. It is that they changed subject. <code>Skills</code> moves furthest of anything
measured, 0.590: in 2024 its neighbours are <em>goals</em>, <em>abilities</em>,
<em>experiences</em> — the vocabulary of what a model can do. By 2026 they are
<em>middleware</em>, <em>reusable</em>, <em>ide</em>, <em>filesystem</em>. The word stopped
describing a capability and started describing a file on disk that an agent loads.</p>

<p>Two of the low-drift rows are the control. <code>Context</code> barely moves at 0.170 —
window, length, cache — and <code>reasoning</code> at 0.285, which is the more interesting one:
the <em>amount</em> of discussion about reasoning changed enormously, as the chapter on it
shows, while the word's meaning barely shifted at all. Volume and meaning are independent, and
a series that tracks one tells you nothing about the other.</p>

<p class="pull">Re-derive what your metrics mean, on a schedule. A dashboard is a set of strings
...
def pages():
    ...
    ch7 = (CH7
           ...
           + table(["Term", "Drift", "Nearest neighbours, 2024",
                   ...
                   "Table 8 · Cosine distance between a term's 2024 and 2026 "
                   "neighbourhoods. Median across the corpus: 0.313", (1,))
           + CH7_DRIFT_B
           + CH7_B + CH7_C
